...
- DB path: `/Users/_xvadur/singularitas/data/crm/pcrm.sqlite`
- Workspace folder: `/Users/_xvadur/singularitas/data/crm`
- Access mode: prefer SQLite read-only URI when inspecting live state to avoid lock contention
...
- List open reminders (next 7 days):
  - `sqlite3 'file:/Users/_xvadur/singularitas/data/crm/pcrm.sqlite?mode=ro' "SELECT id, title, due_at, status FROM reminders WHERE status='open' AND (due_at IS NULL OR due_at <= datetime('now','+7 day')) ORDER BY due_at IS NULL, due_at;"`
- List latest interactions:
  - `sqlite3 'file:/Users/_xvadur/singularitas/data/crm/pcrm.sqlite?mode=ro' "SELECT id, contact_id, channel, direction, subject, at FROM interactions ORDER BY datetime(at) DESC LIMIT 20;"`
- List priority contacts (manual signal):
  - `sqlite3 'file:/Users/_xvadur/singularitas/data/crm/pcrm.sqlite?mode=ro' "SELECT id, name, company, relationship_tier, openclaw_status, last_sync FROM contacts ORDER BY datetime(last_sync) DESC LIMIT 50;"`
